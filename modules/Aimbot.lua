-- Aimbot на голову/туловище/ноги + MINI UI + Full Target + ПОЛНЫЙ ФИКС Aim toggle (работает на ВСЕХ клавишах) + MOUSE METHOD
...
-- Глобальная функция mousemoverel (работает в большинстве executor'ов)
local mousemoverel = mousemoverel or function(dx, dy)
    -- Альтернативный метод через изменение Camera.CFrame
...
local SETTINGS = {
    ...
    wallCheck     = true,
    fullTarget    = false,
    aimMethod     = "Auto", -- "Auto", "Mouse", "Camera"
    ignoreTeams   = true, -- Игнорировать команды в матче
}
...
local currentGameMode = getGameMode()
...
StatusLabel.Text = "Aimbot: OFF | Часть: Head | FullT: OFF\n" .. BINDINGS.AimKey.Name .. " | Режим: " .. currentGameMode
StatusLabel.Parent = ScreenGui

-- MINI UI (расширенная версия)
local MiniUI = Instance.new("Frame")
MiniUI.Size = UDim2.new(0, 240, 0, 480)
MiniUI.Position = UDim2.new(1, -250, 1, -490)
...
createToggle(180, "Игнорить команды", "ignoreTeams")
...
local AimBindBtn = createBind(215, "Aim (toggle):", "Aim")
local TargetBindBtn = createBind(250, "Target (hold):", "Target")

-- Выбор части тела (исправленные названия для Rivals)
...
for i, part in ipairs(parts) do
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for _, b in ipairs(partButtons) do
            b.BackgroundColor3 = (b.Text == (partDisplayNames[part] or part)) and Color3.fromRGB(0,140,255) or Color3.fromRGB(50,50,70)
        end
        StatusLabel.Text = "Aimbot: " .. (aimbotEnabled and "ON" or "OFF") .. " | Часть: " .. (partDisplayNames[part] or part) .. " | FullT: " .. (SETTINGS.fullTarget and "ON" or "OFF") .. "\nРежим: " .. currentGameMode
    end)
...
-- Выбор метода аима
local aimMethods = {"Auto", "Mouse", "Camera"}
local aimMethodButtons = {}

local AimMethodLabel = Instance.new("TextLabel")
AimMethodLabel.Size = UDim2.new(0.9, 0, 0, 20)
AimMethodLabel.Position = UDim2.new(0.05, 0, 0, 320)
AimMethodLabel.BackgroundTransparency = 1
AimMethodLabel.Text = "Метод аима:"
AimMethodLabel.TextColor3 = Color3.fromRGB(180,180,255)
AimMethodLabel.Font = Enum.Font.Gotham
AimMethodLabel.TextSize = 13
AimMethodLabel.Parent = MiniUI

for i, method in ipairs(aimMethods) do
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(0.28, 0, 0, 28)
    btn.Position = UDim2.new(0.05 + (i-1)*0.32, 0, 0, 340)
    btn.BackgroundColor3 = (SETTINGS.aimMethod == method) and Color3.fromRGB(140,0,255) or Color3.fromRGB(50,50,70)
    btn.Text = method
    btn.TextColor3 = Color3.new(1,1,1)
    btn.Font = Enum.Font.GothamSemibold
    btn.TextSize = 12
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for _, b in ipairs(aimMethodButtons) do
            b.BackgroundColor3 = (b.Text == method) and Color3.fromRGB(140,0,255) or Color3.fromRGB(50,50,70)
        end
        print("Метод аима установлен: " .. method)
    end)
    
    table.insert(aimMethodButtons, btn)
end

-- FOV TextBox
local FovLabel = Instance.new("TextLabel")
FovLabel.Size = UDim2.new(0.9, 0, 0, 20)
FovLabel.Position = UDim2.new(0.05, 0, 0, 375)
...
FovBox.Position = UDim2.new(0.05, 0, 0, 395)
...
SmoothLabel.Position = UDim2.new(0.05, 0, 0, 430)
...
SmoothBox.Position = UDim2.new(0.05, 0, 0, 450)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if isAimKey then
        ...
        StatusLabel.Text = "Aimbot: " .. (aimbotEnabled and "ON" or "OFF") .. " | Часть: " .. (partDisplayNames[SETTINGS.targetPart] or SETTINGS.targetPart) .. " | FullT: " .. (SETTINGS.fullTarget and "ON" or "OFF") .. "\nРежим: " .. currentGameMode .. " | Метод: " .. SETTINGS.aimMethod
...
ResetAim.Position = UDim2.new(0.95, 0, 0, 215)
...
ResetTarget.Position = UDim2.new(0.95, 0, 0, 250)
...
-- Улучшенная функция получения части тела
local function getTargetPart(player)
    if not player or not player.Character then return nil end
    
    local char = player.Character
    
    -- Сначала пробуем получить нужную часть
    local part = char:FindFirstChild(SETTINGS.targetPart)
    if part then return part end
    
    -- Если не нашли, ищем альтернативы
    local alternatives = {}
    ...
    -- Если всё ещё не нашли, берем первую доступную BasePart
...
-- isValidTarget (с исправлением для команд)
local function isValidTarget(player)
    ...
    -- Проверка команд с учетом настройки ignoreTeams
    if not SETTINGS.ignoreTeams then
...
-- findNearestEnemy + Full Target (исправленная)
...
RunService.Heartbeat:Connect(function()
    ...
    if not targetPlayer then 
        lockedTarget = nil
        StatusLabel.Text = "Aimbot: ON | Цель не найдена | Режим: " .. currentGameMode .. " | Метод: " .. getActiveAimMethod()
        return 
    end
    ...
    -- Обновляем статус
    StatusLabel.Text = "Aimbot: ON | Цель: " .. targetPlayer.Name .. " | Часть: " .. (partDisplayNames[SETTINGS.targetPart] or SETTINGS.targetPart) .. "\nРежим: " .. currentGameMode .. " | Метод: " .. aimMethod
    
    -- Применяем выбранный метод аима
...
print("⭐ Добавлен MOUSE метод для работы в матче!")
print("")
print("Основные улучшения:")
print("1. Метод мыши для матча (Mouse Method)")
print("2. Автоматическое определение режима игры")
print("3. Исправленные названия частей тела для Rivals")
print("4. Настройка игнорирования команд")
print("5. 3 режима работы: Auto/Mouse/Camera")
print("")
print("Управление:")
print("- " .. BINDINGS.AimKey.Name .. " - Включить/выключить аимбот")
print("- " .. (BINDINGS.TargetKey.Name or "RMB") .. " - Удержание цели (если включено)")
print("- В MINI UI можно выбрать метод аима:")
print("  • Auto - автоматический выбор (мышь в матче, камера в лобби)")
print("  • Mouse - всегда использовать мышь")
print("  • Camera - всегда использовать камеру")
print("")
print("Рекомендации:")
print("- В матче используйте метод 'Mouse' или 'Auto'")
print("- Включите 'Игнорить команды' для работы в матче")
print("- Для точного аима установите FOV 80-120")
print("")

-- Автопроверка при запуске
task.delay(3, function()
    print("Проверка окружения...")
    print("Режим игры: " .. currentGameMode)
    print("Количество игроков: " .. #Players:GetPlayers())
    print("Рекомендуемый метод аима: " .. (currentGameMode == "Match" and "Mouse" or "Camera"))
