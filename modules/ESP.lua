...
function ESP.Init(config)
    ...
    local function updateESP()
        ...
        if now % 1.5 < 0.2 then
            for _, player in ipairs(Players:GetPlayers()) do
                if player.Character and isValidTarget(player) then
                    ...
                    local elements = {}
                    if ESP.Config.BoxEnabled then elements.box = createDrawing("Box") end
                    if ESP.Config.TracerEnabled then elements.tracer = createDrawing("Tracer") end
                    if ESP.Config.NameEnabled then elements.name = createDrawing("Name") end
        ...
        for player, elements in pairs(espElements) do
            ...
            if dist > ESP.Config.MaxRenderDistance then
                for _, obj in pairs(elements) do if obj then obj.Visible = false end end
                continue
            end
            ...
            if ESP.Config.NameEnabled and elements.name then
                local nameText = player.Name
                if ESP.Config.ShowDistance then nameText = nameText .. " [" .. math.floor(dist) .. "]" end
